                ...
                <PerfectScrollbar options={{ suppressScrollX: true, wheelPropagation: false }}>
                    <div className='bill-details'>
                        <table>
                            ...
                            <tbody>
                                {cart.map((item, index) => <tr key={index}><td colSpan={2} className='pname'>{item.title}</td><td>{item.itemUnit === 'g' || item.itemUnit === 'ml' ? item.itemUnitValue * 1000 : item.itemUnitValue} {item.itemUnit}</td><td style={{textAlign:'right'}}>{priceDisplay(parseInt(item.price)).replace("₹", "")}</td><td style={{textAlign:'right'}}>{priceDisplay(parseInt(item.totalPrice)).replace("₹", "")}</td></tr>)}
